...
Зайдите на aistudio.google.com.
...
Нам нужно добавить библиотеку для работы с Google AI.
pip install langchain-google-genai

Шаг 3: Финальный RAG-скрипт
Этот скрипт объединяет всё вместе:

Принимает вопрос.

Лезет в вашу базу vk_vector_db.

Находит релевантные посты.

Отправляет их вместе с вопросом в LLM, чтобы она сформулировала ответ.

Возможные улучшения
Память диалога: Сейчас бот не помнит предыдущий вопрос. Чтобы это исправить, в LangChain нужно добавить ConversationBufferMemory.

Другая модель (Если нужен GigaChat): Если вы хотите полностью российское решение, замените импорт и инициализацию llm:

from langchain_community.chat_models import GigaChat

llm = GigaChat(credentials="ВАШ_ТОКЕН_АВТОРИЗАЦИИ", verify_ssl_certs=False)
(Для этого нужно получить токен на сайте разработчиков Сбера).
